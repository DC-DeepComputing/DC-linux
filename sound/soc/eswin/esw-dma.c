										  ...
										  struct snd_pcm_substream *substream)
{
	...
	/*
	 * copy() widens 16/24-bit samples to 32-bit slots. The cyclic
	 * length must match that stretched buffer (64 bits per frame).
	 */
					 ...
					 struct snd_pcm_substream *substream)
{
	...
	if (!chan) {
		dev_err(chip->dev, "no DMA channel for stream %d\n",
			substream->stream);
		return -ENODEV;
	}
			       ...
			       struct iov_iter *iter, unsigned long bytes)
{
	...
	if (chip->chan[substream->stream] && chip->chan[substream->stream]->device) {
		size_t hw_bytes = bytes;
		dma_addr_t hw_addr = runtime->dma_addr +
			(dma_addr_t)(dma_ptr - (char *)runtime->dma_area);
		...
		if (runtime->sample_bits == 16)
			...
			hw_bytes = bytes / 3 * 4;
		if (is_playback)
			dma_sync_single_for_device(chip->chan[substream->stream]->device->dev,
						   hw_addr, hw_bytes, DMA_TO_DEVICE);
		else
			dma_sync_single_for_cpu(chip->chan[substream->stream]->device->dev,
						hw_addr, hw_bytes, DMA_FROM_DEVICE);
	}
			     ...
			     struct snd_soc_pcm_runtime *rtd)
{
	...
	for_each_pcm_streams(i) {
		...
		chan = chip->chan[substream->stream];
...
int esw_pcm_dma_dai_register(struct i2s_dev *chip)
{
	...
	/*
	 * Without dmas/dma-names (and without AON DMAC on this branch)
	 * both channels stay NULL. Do not register the DMA PCM component:
	 * its pcm_new used to dereference chan->device and Oops during
	 * audio-graph-card bind, hanging boot.
	 */
	if (!chip->chan[SNDRV_PCM_STREAM_PLAYBACK] &&
	    !chip->chan[SNDRV_PCM_STREAM_CAPTURE]) {
		dev_warn(chip->dev,
			 "no I2S DMA channels; skip PCM DMA component\n");
		return 0;
	}

	/*
	 * ASoC debugfs keys off component name. Two I2S blocks must
	 * not both register as "esw-dma" on the same card.
	 */
	chip->pcm_driver = esw_pcm_component_driver;
	if (chip->dev->of_node)
		snprintf(chip->pcm_name, sizeof(chip->pcm_name), "esw-dma-%s",
			 chip->dev->of_node->name);
	else
		strscpy(chip->pcm_name, "esw-dma", sizeof(chip->pcm_name));
	chip->pcm_driver.name = chip->pcm_name;

	ret = snd_soc_component_initialize(&chip->pcm_component,
					   &chip->pcm_driver, chip->dev);
	if (ret)
		goto release_buf1;
...
{
	if (chip->pcm_component.driver)
		snd_soc_unregister_component_by_driver(chip->dev,
						       chip->pcm_component.driver);
