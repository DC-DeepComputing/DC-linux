		      ...
		      struct snd_pcm_substream *substream)
{
	...
	if (!i2s_drvdata->use_pio) {
		/* Same as 6.6: dedicated TXCH0 + TXDMA_CH(0). */
		i2s_enable_dedicated_dma(i2s_drvdata, substream->stream,
					 config->chan_nr);
	}
		...
		struct snd_soc_dai *cpu_dai)
{
	...
	/* 6.6: start DMA then I2S; stop I2S then DMA. */
	dai_link->trigger_start = SND_SOC_TRIGGER_ORDER_DEFAULT;
	dai_link->trigger_stop = SND_SOC_TRIGGER_ORDER_DEFAULT;
...
static int i2s_reset(struct platform_device *pdev, struct i2s_dev *i2s)
{
	...
	/* Only i2s0 remuxes the HDMI-shared I2S0 pads. i2s1 must still
	 * take the block reset, but must not steal i2s0 pins.
	 */
	if (of_node_name_prefix(pdev->dev.of_node, "i2s0") &&
	    !of_property_read_bool(pdev->dev.of_node, "io_reuse_enable")) {
...
static int i2s_probe(struct platform_device *pdev)
{
	...
	/* Shared VO I2S resets are exclusive. Only i2s0 may take them. */
	if (of_node_name_prefix(pdev->dev.of_node, "i2s0")) {
...
static void i2s_remove(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(pdev->dev.of_node, "dsp_sof_enable")) {
		if (i2s_drvdata->chan[0])
			dma_unmap_page_attrs(i2s_drvdata->chan[0]->device->dev, i2s_drvdata->rb_out_iova, DSP_RB_DATA_SZIE,
								DMA_BIDIRECTIONAL, DMA_ATTR_SKIP_CPU_SYNC);
		if (i2s_drvdata->chan[1])
