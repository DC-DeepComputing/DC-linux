...
static u16 find_gamma_lut(int ave_gamma, u16 index);
static int find_min_in_array(const int *array, size_t size);
static u32 eswin_log2(u32 x);
static void update_roi(struct es_dc *dc, enum dc_hw_plane_id id,
			     ...
			     struct drm_plane_state *state)
{
	...
	crtc_state = drm_atomic_get_old_crtc_state(state->state, crtc);
