...
es_fb_alloc(struct drm_device *dev, const struct drm_format_info *info, 
...
es_fb_create(struct drm_device *dev, struct drm_file *file_priv, 
		const struct drm_format_info *info, const struct drm_mode_fb_cmd2 *mode_cmd)
{
	struct drm_framebuffer *fb;
	...
	fb = es_fb_alloc(dev, info, mode_cmd, objs, i);
	if (IS_ERR(fb)) {
		ret = PTR_ERR(fb);
		goto err;
	}
