...
extern const struct dw_hdmi_property_ops dw_hdmi_eswin_property_ops;
			 ...
			 const struct drm_display_mode *mode)
{
	...
	hdmi->hdmi_data.enc_out_encoding =
		dw_hdmi_eswin_get_enc_out_encoding(data);
	...
	hdmi->hdmi_data.enc_in_bus_format =
		dw_hdmi_eswin_get_input_bus_format(data);
	...
	hdmi->hdmi_data.enc_out_bus_format =
		dw_hdmi_eswin_get_output_bus_format(data);
	...
	unsigned long *enc_out_encoding, unsigned int *eotf)
{
	...
	*color_format = DRM_HDMI_OUTPUT_DEFAULT_RGB;
	...
	case DRM_HDMI_OUTPUT_YCBCR_HQ:
		...
		else if (info->color_formats & (1 << DRM_OUTPUT_COLOR_FORMAT_YCBCR422))
	...
	case DRM_HDMI_OUTPUT_YCBCR_LQ:
		if (conn_state->connector->ycbcr_420_allowed &&
			...
			*color_format = DRM_HDMI_OUTPUT_YCBCR422;
		else if (info->color_formats & (1 << DRM_OUTPUT_COLOR_FORMAT_YCBCR444))
			*color_format = DRM_HDMI_OUTPUT_YCBCR444;
		break;
	...
	*eotf = TRADITIONAL_GAMMA_SDR;
	...
	if (conn_state->hdr_output_metadata) {
		...
		hdr_match = hdr_metadata->hdmi_metadata_type1.eotf & BIT(*eotf);
	}

	if ((*eotf > TRADITIONAL_GAMMA_HDR && hdr_match) || (hdmi->colorimetry == ESWIN_HDMI_COLORIMETRY_BT2020))
	...
	info->max_tmds_clock = min(info->max_tmds_clock, 594000);

	if (tmdsclock > info->max_tmds_clock) {
		if (info->max_tmds_clock >= 594000) {
			*color_depth = 8;
		} else if (info->max_tmds_clock > 340000) {
	...
	if (force == DRM_FORCE_OFF) {
		...
		if (!hdmi->bridge_is_on) {
			...
			if (drm_edid == NULL)
				...
				/* Update display_info from EDID so SCDC/scrambling
				 * configuration is correct during hot-plug. Without this,
				 * display_info gets zeroed after unplug and SCDC is skipped,
				 * causing 4K@60 (594MHz TMDS) to fail on die1. */
	...
	struct drm_property *property, uint64_t val)
{
	...
	const struct dw_hdmi_property_ops *ops = &dw_hdmi_eswin_property_ops;
