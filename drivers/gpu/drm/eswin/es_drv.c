...
static struct drm_driver es_drm_driver = {
	...
	.fbdev_probe = eswin_drm_fbdev_create,
};
...
static int es_drm_bind(struct device *dev)
{
	...
	#else
	eswin_drm_fbdev_client_setup(drm_dev, format);
	#endif
...
static const struct dev_pm_ops es_drm_pm_ops = {
	SYSTEM_SLEEP_PM_OPS(es_drm_suspend, es_drm_resume)
};

static const struct of_device_id es_drm_dt_ids[] = {

	{
		.compatible = "eswin,eic7700-display-subsystem",
	},
